...
# -*- coding: utf-8 -*-
"""
probar_prezioso_y_regresiones_30curvas.py

Extiende reconstruccion_beta_optimo_18curvas/probar_prezioso_y_regresiones.py
a las 30 curvas de resultados_30curvas.csv (10 INL x 3 pulsos). Mismo
metodo exacto (ver docstring del script original) -- solo cambia la fuente
de datos (18->30 filas) y la paleta de colores.

Usa `resultados_30curvas.csv` (generado por reconstruir_corrientes_30curvas.py)
para:
...
Salida (en esta misma carpeta):
  coeficientes_todos_los_modelos_30curvas.csv, resultados_prezioso_30curvas.csv,
  comparacion_R2_todos_los_modelos_30curvas.png, prezioso_literal_vs_empirico_30curvas.png,
  ecuacion_final_vs_empirico_30curvas.png
"""
import os
import csv
...
CSV_IN = os.path.join(HERE, "resultados_30curvas.csv")

COLOR_POR_INL = {
    "2e-1": "#d64a6a", "5e-2": "#e8823f", "1e-2": "#c9a227", "5e-3": "#8bbf3f",
...
def main():
    ...
    csv_prezioso = os.path.join(HERE, "resultados_prezioso_30curvas.csv")
    ...
    coef_path = os.path.join(HERE, "coeficientes_todos_los_modelos_30curvas.csv")
    ...
    ax.set_title(f"Ecuación de Prezioso literal vs. empírico (30 curvas, R²={r2_lit:.3f})")
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "prezioso_literal_vs_empirico_30curvas.png"))
    plt.close(fig)
    ...
    ax.set_ylabel("R² (ajuste log-log a los 30 β óptimo reales)")
    ax.set_title("Comparación de todos los modelos probados (30 curvas)")
    ax.legend(frameon=False, fontsize=9, loc="lower right")
    plt.setp(ax.get_xticklabels(), fontsize=8)
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "comparacion_R2_todos_los_modelos_30curvas.png"))
    plt.close(fig)
    ...
    ax.set_ylabel(r"$\beta_{óptimo}$ empírico")
    ax.set_title(f"Ecuación final vs. empírico (30 curvas, R²={mejor['r2']:.3f})")
    for tag, c in COLOR_POR_INL.items():
        ax.scatter([], [], color=c, label=f"INL {tag}")
    for p, m in MARKER_POR_P.items():
        ax.scatter([], [], color="#555", marker=m, label=f"p={p}")
    ...
    ax.grid(True, which="both", color="#e8e7e0", linewidth=0.6)
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "ecuacion_final_vs_empirico_30curvas.png"))
    plt.close(fig)
    ...
    print(f"\nError absoluto porcentual: media={np.mean(errores):.1f}%  mediana={np.median(errores):.1f}%  "
          f"max={np.max(errores):.1f}%")

    print("\nGuardadas figuras: prezioso_literal_vs_empirico_30curvas.png, "
          "comparacion_R2_todos_los_modelos_30curvas.png, ecuacion_final_vs_empirico_30curvas.png")
