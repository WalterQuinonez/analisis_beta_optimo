...
para INL=1e-2, p=200 (data_beta_grid_search_SP/beta_grid_search_SP_INL_1e-2_p_200/),
para cada uno de los 40 valores de beta de la grilla.

Version "hermana" de analisis_distribucion_softmax_INL_2e-1_p_{50,100,200}/
(mismo metodo exacto), esta vez con INL=1e-2 -una curva P/D MUCHO mas
lineal que INL=2e-1- para responder el "proximo paso" del reporte de
INL=2e-1: ver si el rango de logit_std en el beta optimo se mantiene, y si
el "acantilado de estabilidad" (colapso post-optimo) es propio de INL
grande (muy no lineal) o aparece tambien, mas atenuado, en curvas casi
lineales.

Objetivo: ver si la distribucion softmax en el beta OPTIMO empirico
(beta=750, acc=91.63%, ver
...
CARPETA = os.path.join(REPO_ROOT, "data_beta_grid_search_SP", "beta_grid_search_SP_INL_1e-2_p_200")
SERIE = 0
BETA_OPTIMO_EMPIRICO = 750.0  # resumen_por_INL/resumen_optimos.csv: INL=1e-2, p=200
BETAS_REPRESENTATIVOS = [1.0, 50.0, 100.0, 200.0, 300.0, 750.0, 800.0, 1950.0]
TITULO_CURVA = "INL=1e-2, p=200"
...
colores_rep = {1.0: "#7a7a7a", 50.0: "#4ac9c2", 100.0: "#d64a6a", 200.0: "#c9a227",
               250.0: "#8b5cf6", 300.0: "#3aa66b", 350.0: "#e0559b", 750.0: "#00838f",
