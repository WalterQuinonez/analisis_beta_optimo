#!/bin/bash
#SBATCH --job-name=beta_grid_INL_7e-2_p200
...
# existir de antemano: `mkdir -p beta_grid_search_SP_INL_7e-2_p_200`
# antes de este sbatch).
#SBATCH --output=beta_grid_search_SP_INL_7e-2_p_200/slurm_beta_%A_%a.out
#SBATCH --error=beta_grid_search_SP_INL_7e-2_p_200/slurm_beta_%A_%a.err
#
...
# ---------------------------------------------------------------------------
OUT_DIR="beta_grid_search_SP_INL_7e-2_p_200"    # <- carpeta de resultados de ESTA corrida
PULSOS_POT=200                   # <- puntos de la curva de potenciacion
PULSOS_DEP=200                   # <- puntos de la curva de depresion
A_POT=65.12                     # <- 'a' de potenciacion (mismo INL que arriba)
A_DEP=65.12                     # <- 'a' de depresion   (mismo INL que arriba)
#
