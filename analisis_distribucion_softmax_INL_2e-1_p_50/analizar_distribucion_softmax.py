...
para INL=2e-1, p=50 (data_beta_grid_search_SP/beta_grid_search_SP_INL_2e-1_p_50/),
para cada uno de los 40 valores de beta de la grilla.

Objetivo: ver si la distribucion softmax en el beta OPTIMO empirico
(beta=100, acc=84.79%, ver data_beta_grid_search_SP/resumen_por_INL/resumen_optimos.csv)
tiene algo particular frente a betas mas chicos (softmax casi uniforme,
subconfiado) o mas grandes (softmax saturado, sobreconfiado), o si al menos
hay una regularidad util para comparar mas adelante contra otros p / INL.
...
CARPETA = os.path.join(REPO_ROOT, "data_beta_grid_search_SP", "beta_grid_search_SP_INL_2e-1_p_50")
SERIE = 0
BETA_OPTIMO_EMPIRICO = 100.0  # resumen_por_INL/resumen_optimos.csv: INL=2e-1, p=50
BETAS_REPRESENTATIVOS = [1.0, 50.0, 100.0, 300.0, 800.0, 1950.0]
...
ax.set_title("INL=2e-1, p=50 — accuracy reconstruida vs beta")
...
ax1.set_title("INL=2e-1, p=50 — confianza y entropia del softmax vs beta")
...
fig.suptitle("INL=2e-1, p=50 — metricas de calibracion vs beta")
fig.tight_layout()
...
colores_rep = {1.0: "#7a7a7a", 50.0: "#4ac9c2", 100.0: "#d64a6a", 300.0: "#3aa66b",
               800.0: "#eb6834", 1950.0: "#2a78d6"}
...
ax.set_title("INL=2e-1, p=50 — distribucion de confianza softmax por beta")
ax.legend(frameon=False, fontsize=9)
ax.spines[["top", "right"]].set_visible(False)
fig.tight_layout()
...
ax.set_ylabel("densidad")
ax.set_title("INL=2e-1, p=50 — distribucion de logits crudos por beta\n"
...
fig, axes = plt.subplots(2, 3, figsize=(13, 8), dpi=140, sharex=True, sharey=True)
for ax, b in zip(axes.ravel(), BETAS_REPRESENTATIVOS):
    if b not in distribuciones:
        ax.axis("off")
        continue
...
fig.suptitle("INL=2e-1, p=50 — diagramas de reliability por beta\n"
